...
def filter_highest_clip_columns(d):
    """
    Given a dictionary d with model names as keys and numerical scores as values,
    filter out all CLIP models except the one with the highest score.
    The original model name for the highest scoring CLIP entry is preserved.
    """
    # List of candidate CLIP model keys
    clip_models = [
        "CLIP RN50x64", "CLIP ViT-B/32", "CLIP ViT-L/14",
        "openCLIP ViT-H/14", "openCLIP ViT-G/14"
    ]
    
    # Find which of the clip models exist in the dictionary
    existing_clip = [key for key in clip_models if key in d]
    
    if existing_clip:
        # Select the key with the highest score
        best_clip = max(existing_clip, key=lambda k: d[k])
        # Build a new dictionary with all keys not in clip_models
        filtered = {k: v for k, v in d.items() if k not in clip_models}
        # Add the best clip (with its original key)
        filtered["CLIP BEST"] = d[best_clip]
        # filtered["CLIP BESt"]
        return filtered
    else:
        # If no CLIP keys exist, return the original dictionary
        return d



color_correct_scores_first_block = filter_highest_clip_columns(color_correct_scores_first_block)
color_correct_scores_second_block = filter_highest_clip_columns(color_correct_scores_second_block)
color_correct_scores_third_block = filter_highest_clip_columns(color_correct_scores_third_block)
...
df_colors = pd.DataFrame({  
    ...
    'Attribute (Block 1)': color_correct_scores_first_block,
    'Attribute (Block 2)': color_correct_scores_second_block,
    'Attribute (Block 3)': color_correct_scores_third_block,
    
})
color_attribute_scores_first_block = filter_highest_clip_columns(color_attribute_scores_first_block)
color_attribute_scores_second_block = filter_highest_clip_columns(color_attribute_scores_second_block)
color_attribute_scores_third_block = filter_highest_clip_columns(color_attribute_scores_third_block)

df_color_attributes = pd.DataFrame({
'Attribute (Block 1)': color_attribute_scores_first_block,
    'Attribute (Block 2)': color_attribute_scores_second_block,
    'Attribute (Block 3)': color_attribute_scores_third_block,
    })

counting_correct_scores_first_block = filter_highest_clip_columns(counting_correct_scores_first_block)
counting_correct_scores_second_block = filter_highest_clip_columns(counting_correct_scores_second_block)
counting_correct_scores_third_block = filter_highest_clip_columns(counting_correct_scores_third_block)
...
single_object_correct_first_block=  filter_highest_clip_columns(single_object_correct_first_block)
single_object_correct_second_block = filter_highest_clip_columns(single_object_correct_second_block)
single_object_correct_third_block = filter_highest_clip_columns(single_object_correct_third_block)
...
df_object = pd.DataFrame({
    ...
    'Object (Block 1)': single_object_correct_first_block,
    'Object (Block 2)': single_object_correct_second_block,
    'Object (Block 3)': single_object_correct_third_block,
    
})

position_correct_scores_first_block = filter_highest_clip_columns(position_correct_scores_first_block)
position_correct_scores_second_block = filter_highest_clip_columns(position_correct_scores_second_block)
position_correct_scores_third_block = filter_highest_clip_columns(position_correct_scores_third_block)
...
two_objects_correct_first_block = filter_highest_clip_columns(two_objects_correct_first_block)
two_objects_correct_second_block = filter_highest_clip_columns(two_objects_correct_second_block)
two_objects_correct_third_block = filter_highest_clip_columns(two_objects_correct_third_block)

df_two = pd.DataFrame({
'Object (Block 1)': two_objects_correct_first_block,
    'Object (Block 2)': two_objects_correct_second_block,
    'Object (Block 3)': two_objects_correct_third_block,
})

df_attribute = pd.concat([df_colors, df_color_attributes]).groupby(level=0).mean()
# df_attribute = df_attribute.mean(axis=1)
# print(df_colors)
# print(df_color_attributes)

df_object = pd.concat([df_object, df_two]).groupby(level=0).mean()

combined_df = pd.concat([df_attribute,df_counting, df_object, df_spatial], axis=1)
# Extract columns containing "Block 1"
block1_df = combined_df.filter(like="Block 1", axis=1)
block1_df = block1_df.rename(columns=lambda x: x.replace(" (Block 1)", ""))

block2_df = combined_df.filter(like="Block 2", axis=1) 
block2_df = block2_df.rename(columns=lambda x: x.replace(" (Block 2)", ""))

block3_df = combined_df.filter(like="Block 3", axis=1)
block3_df = block3_df.rename(columns=lambda x: x.replace(" (Block 3)", ""))
...
def create_bar_plot(df, category_name):
    ...
    # Color schemes
    clip_colors = ["CLIP RN50x64", "CLIP ViT-B/32", "CLIP ViT-L/14", "openCLIP ViT-H/14", "openCLIP ViT-G/14"]
    # df = df.drop when clip_colors are in the columns
    df_transposed = df_transposed.drop(clip_colors, axis=1, errors='ignore')
    # colorblind_palette = sns.color_palette("colorblind", len(clip_colors))
    ...
    # Group models
    # clip_models = [model for model in df_transposed.columns if model in clip_colors]
    clip_models = [model for model in df_transposed.columns if model in colorblind_pallete]
    ...
    index = np.arange(len(df_transposed.index)) * (5 * bar_width + group_spacing)
    
    # print(len(df_transposed.index))
    ...
    clip_offset = 0
    gap = 0.0  # Small gap between clip and generation
    gen_offset = 1 + gap  # Generation after Clip with a gap
...
# Plot Clip Models
    for i, model in enumerate(clip_models):
        bars = plt.bar(index + (clip_offset) * bar_width, 
                df_transposed[model], 
                bar_width, 
                label=model, 
                color=colorblind_pallete[model], 
                zorder=10)
        for bar in bars:
            yval = bar.get_height()
            plt.text(bar.get_x() + bar.get_width()/2.0, yval,  int(round(yval, 2)*100), va='bottom', ha='center', fontsize=18)
    ...
    if 'Generation' in df_transposed.columns:
        if df_transposed['Generation'].notna().all():  # Check if it has values
            bars = plt.bar(index + (gen_offset * bar_width), 
    ...
    for j, model in enumerate(sd_models):
        bars = plt.bar(index + (sd_offset + j) * bar_width, 
                df_transposed[model], 
                bar_width, 
                label=model, 
                color=sd_colors[j], 
    ...
    xtick_label = ['Attribute', 'Counting', 'Object', 'Position']
    ...
    plt.tight_layout()
    outdir = 'plots/ours_shortened_combined/'
    os.makedirs(f'{outdir}', exist_ok=True)

    plt.savefig(f'{outdir}/{category_name}_scores_plot_ours.png', dpi=300, bbox_inches='tight')
    # plt.show()
...
create_bar_plot(block1_df, "Block 1")
create_bar_plot(block2_df, "Block 2")
create_bar_plot(block3_df, "Block 3")