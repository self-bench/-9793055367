...
def main():
    ...
    # for index, row in tqdm(data.iloc[::-1].iterrows(), total=data.shape[0], desc="Reverse Progress"):
    shuffled_indices = data.index.to_list()
    random.shuffle(shuffled_indices)

    # Iterate over the DataFrame in random order while keeping the original index
    for index in tqdm(shuffled_indices, total=data.shape[0]):
        row = data.loc[index]
        pos_url = row['pos_url']
        neg_url = row['neg_url']
