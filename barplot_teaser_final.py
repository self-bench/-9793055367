...
brightness_values = [1.0, 0.7, 0.5]
color_pallete = [mcolors.hsv_to_rgb((base_hsv[0], base_hsv[1], brightness_values[i])) for i in range(3)]
# x = 
# x = 
...
for bar in bars_geneval:
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width() / 2, height,
            f'{height*100:.0f}%',  # Display with 2 decimal places
...
plt.yticks(np.linspace(0, 1, 5),fontsize=1)  # Change the Y-axis tick label size
plt.yticks([])
...
ax.set_xticklabels(["SD 1.5", "SD 2.0", "SD 3-m"], fontsize=15)
plt.tick_params(axis='x', which='both', bottom=False, top=False)
plt.yticks(np.linspace(0, 1, 5),fontsize=1)
# plt.yticks(np.linspace(0, 0.8, 5),fontsize=15)  # Change the Y-axis tick label size
plt.yticks([])
ax.set_ylabel('Accuracy', fontsize=15)
...
plt.savefig(f'{outdir}/disc_others.png', dpi=300, bbox_inches='tight')
...
# print(df_zero_shot)
# exit(0)
zero_shot_values = [df_zero_shot[col].iloc[0] for col in x]
...
# x = 
# Create the bars for each category:
bars_geneval = ax.bar(np.arange(len(x)), zero_shot_values, bar_width,
                      color=[color_pallete[i] for i in range(3)],
                      label='Geneval', zorder=10)
for bar in bars_geneval:
    height = bar.get_height()
...
plt.savefig(f'{outdir}/disc_zero.png', dpi=300, bbox_inches='tight')
df_self_all = pd.DataFrame({

    "SD 1.5": [(100+90+98+83+67+76)/6],
    "SD 2.0": [(100+98+98+88+84+95)/6],
    "SD 3-m": [(100+98+98+95+89+91)/6],
})

zero_shot_values = [df_self_all[col].iloc[0] for col in x]
fig, ax = plt.subplots(figsize=(3, 2))
...
for bar in bars_geneval:
    height = bar.get_height()
    ax.text(bar.get_x() + bar.get_width() / 2, height,
            f'{height:.0f}%',  # Display with 2 decimal places
            ha='center', va='bottom', fontsize=15, color='black')
...
plt.savefig(f'{outdir}/disc_self.png', dpi=300, bbox_inches='tight')
