...
def create_bar_plot(df_transposed, category_name):
    ...
    df_transposed_generation = df_transposed[['Generation_single', 'Generation_two', 'Generation_colors', 'Generation_color_attr', 'Generation_position', 'Generation_counting']]


    num_groups = len(df_transposed_full.columns)
    x = np.arange(num_groups)  # positions for each category

    categories = ["Single Object", "Two Objects", "Colors", "Color Attribute", "Position", "Counting"]
    ...
    generation_values = df_transposed_generation.iloc[0].values.astype(float)  # shape (6,)
    ...
    bars_full = ax.bar(x - bar_width, full_values, bar_width, label="Full", color=color_blind_pallete[0], zorder=10)
    bars_correct = ax.bar(x, correct_values, bar_width, label="Correct", color=filtered_palette[4], zorder=10)
    bars_generation = ax.bar(x + bar_width+0.00002, generation_values, generation_bar_width, label="Generation", color=color_blind_pallete[4], zorder=10)
    ...
    plt.tight_layout()
    outdir = 'plots/full_correct_generation'
