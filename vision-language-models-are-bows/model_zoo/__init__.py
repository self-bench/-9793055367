...
def get_model(model_name, device, root_dir=CACHE_DIR):
    ...
    if "openai-clip" in model_name:
        ...
        variant = model_name.split(":")[1]
        if "H/14" in model_name:
            model, _, image_preprocess = open_clip.create_model_and_transforms(model_name.split(":")[1], pretrained="laion2b_s32b_b79k", device=device)
        elif "g/14" in model_name:
            model, _, image_preprocess = open_clip.create_model_and_transforms(model_name.split(":")[1], pretrained="laion2b_s34b_b88k", device=device) 
        else: model, _, image_preprocess = open_clip.create_model_and_transforms(model_name=variant, pretrained="laion2b_s34b_b79k", device=device)
        model = model.eval()
        clip_model = CLIPWrapper(model, device) 
        return clip_model, image_preprocess
