...
class CLIPWrapper:
    ...
    def get_image_scores_dataset(self, loader):
    ...
    def get_retrieval_scores_batched(self, joint_loader):
        ...
        for batch in tqdm_loader:
            ...
            for c_option in batch["caption_options"]:
                ...
                # print(self.model.encode_text(caption_tokenized.to(self.device)).dtype)
            ...
            print(self.model(batch["image_options"][0].to(self.device), torch.cat([clip.tokenize('a photo of 4 eggs')]).to(self.device))[1])
            print(self.model(batch["image_options"][1].to(self.device), torch.cat([clip.tokenize('a photo of 4 eggs')]).to(self.device))[1])

            print(self.model(batch["image_options"][0][0].unsqueeze(0).to(self.device), torch.cat([clip.tokenize('a photo of 4 eggs')]).to(self.device))[1])
            print(self.model(batch["image_options"][1][0].unsqueeze(0).to(self.device), torch.cat([clip.tokenize('a photo of 4 eggs')]).to(self.device))[1])

            print(self.model(batch["image_options"][0][1].unsqueeze(0).to(self.device), torch.cat([clip.tokenize('a photo of 4 eggs')]).to(self.device))[1])
            print(self.model(batch["image_options"][1][1].unsqueeze(0).to(self.device), torch.cat([clip.tokenize('a photo of 4 eggs')]).to(self.device))[1])

            print(self.model(batch["image_options"][0][2].unsqueeze(0).to(self.device), torch.cat([clip.tokenize('a photo of 4 eggs')]).to(self.device))[1])
            print(self.model(batch["image_options"][1][2].unsqueeze(0).to(self.device), torch.cat([clip.tokenize('a photo of 4 eggs')]).to(self.device))[1])
            ...
            batch_scores = np.einsum("nkd,nld->nkl", image_options, caption_options) #* logit_scale_np
            # print(batch_scores)
            # exit(0)
