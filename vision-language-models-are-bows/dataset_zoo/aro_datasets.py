...
class VG_Relation(Dataset):
    ...
    def evaluate_scores(self, scores):
        ...
        result_records.append({'Relation': 'Total', 'I2T Accuracy': metrics['I2T Accuracy'], 'Count': len(all_relations), 'Dataset': 'Visual Genome Relation'})
...
class VG_Attribution(Dataset):
    ...
    def evaluate_scores(self, scores):
        ...
        result_records.append({'Attributes': 'Total', 'Accuracy': np.mean(correct_mask), 'Count': len(all_attributes), 'Dataset': 'Visual Genome Attribution'})
        return result_records
...
class Flickr30k_Order(Dataset):
    def __init__(self, image_preprocess, split, root_dir=FLICKR_ROOT, max_words=30,
        ...
        root_dir = '../../../data/raw/flickr-image-dataset/versions/1'
        if not os.path.exists(root_dir):
    ...
    def __getitem__(self, index):  
        ...
        image_path = image_path.replace("flickr30k-images", "flickr30k_images/flickr30k_images")       
        image = Image.open(image_path).convert('RGB')    
