...
# print(df_object_ours_3m)
# print(df_spatial_ours_3m)
# exit(0)

df_all_ours_3m_no_Std = pd.concat([df_object_ours_3m, df_attribute_ours_3m, df_spatial_ours_3m, df_counting_ours_3m], axis=1)

df_all_ours_3m_no_Std = df_all_ours_3m_no_Std.drop(columns=["Position Std", "Counting Std"])
# print(['')
print(df_all_ours_3m_no_Std.mean(axis = 1))
exit(0)
...
def create_bar_plot(df, category_name):
    ...
    for j, model in enumerate(sd_models):
        print(df_std_transposed[model])
        if category_name=="All_Ours":
            bars = plt.bar(index + ( j) * bar_width, df_transposed[model], bar_width, label=model, color=sd_colors[j], zorder=10)
        else: bars = plt.bar(index + ( j) * bar_width, df_transposed[model], bar_width, label=model, color=sd_colors[j], zorder=10,
    ...
    plt.yticks(fontsize=20)  # Change the Y-axis tick label size
    ...
    plt.tight_layout()
    outdir = 'plots/teaser_average_ours'
