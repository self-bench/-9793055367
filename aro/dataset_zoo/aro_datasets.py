...
class VG_Relation(Dataset):
    def __init__(self, image_preprocess, text_perturb_fn=None, image_perturb_fn=None, root_dir=ARO_ROOT, download=False, resize=512):
        ...
        # self.root_dir = "../../../../data/raw/vgr_vga"
        annotation_file = os.path.join(self.root_dir, "visual_genome_relation.json")
    ...
    def evaluate_scores(self, scores):
        ...
        metrics = {"I2T Accuracy": None, "T2I Accuracy": None}
        preds = np.argmax(np.squeeze(scores_i2t, axis=1), axis=-1)
        preds_t2i = np.argmax(np.squeeze(scores_t2i, axis=1), axis=-1)
        correct_mask = (preds == 1)
        correct_mask_t2i = (preds_t2i == 1)
        metrics["I2T Accuracy"] = np.mean(correct_mask)
        metrics["T2I Accuracy"] = np.mean(correct_mask_t2i)
        ...
        for relation in np.unique(all_relations):
            ...
            result_records.append({
                "Relation": relation,
                "I2T Accuracy": correct_mask[relation_mask].mean(),
                "T2I Accuracy": correct_mask_t2i[relation_mask].mean(),
...
class VG_Attribution(Dataset):
    ...
    def evaluate_scores(self, scores):
        ...
        result_records.append({'Relation': 'Total', 'Accuracy': correct_mask.mean(), 'Count': len(all_attributes), 'Dataset': 'Visual Genome Attribution'})
        return result_records
...
class COCO_Order(Dataset):
    ...
    def __getitem__(self, index):  
        ...
        if self.image_preprocess is None:
            ...
            img0_resize = self.image_preprocess(image)
        # print("length", len(test_case["caption_options"]))
        # exit(0)
...
class Flickr30k_Order(Dataset):
    ...
    def __getitem__(self, index):  
        test_case = self.test_cases[index]  
        image_path = os.path.join(self.root_dir, test_case["image"].replace('flickr30k-images/', ''))   
