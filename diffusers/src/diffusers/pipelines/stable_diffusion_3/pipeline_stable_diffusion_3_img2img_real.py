...
# from vqvae._post.self_bench.diffusers.src.diffusers.models.ip_adapters import SD3IPAdapterMixin
# from vqvae._post.self_bench.diffusers.src.diffusers.models.lora import SD3LoraLoaderMixin
# from vqvae._post.self_bench.diffusers.src.diffusers.loaders.single_file import FromSingleFileMixin
...
def retrieve_timesteps(
    ...
    **kwargs,
    ...
    if timesteps is not None:
        ...
        scheduler.set_timesteps(num_inference_steps, device=device, timesteps=custom_timesteps, **kwargs)
        timesteps = scheduler.timesteps
    return timesteps, num_inference_steps


class StableDiffusion3Img2ImgPipelineReal(DiffusionPipeline):
    r"""
    Args:
    ...
    def prepare_latents(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None):
        ...
        if timestep.item() not in self.model_noises:
            noise = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
            self.model_noises[timestep.item()] = noise
        else:
            noise = self.model_noises[timestep.item()]
            print(f'found noise for timestep {timestep.item()}')
            

        # get latents
        init_latents = self.scheduler.scale_noise(init_latents, timestep, noise)[0]
    ...
    def __call__(
        ...
        timesteps, num_inference_steps = retrieve_timesteps(
            self.scheduler, num_inference_steps, device, sigmas=sigmas, custom_timesteps=custom_timesteps, **scheduler_kwargs
        )
