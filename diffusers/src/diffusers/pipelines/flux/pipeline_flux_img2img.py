...
class FluxImg2ImgPipeline(DiffusionPipeline, FluxLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        generator,
        latents=None,
        sampling_step=None, presaved_latents=None, 
    ):
        ...
        if presaved_latents is None:
            ...
            image_latents = presaved_latents
            model_input = image_latents
        # print(shape)
        # exit(0)
        ...
        if self.scheduler.__class__.__name__ != "FlowMatchEulerDiscreteScheduler":
            ...
            sigma = None
        else: # latents = self.scheduler.scale_noise(image_latents, timestep, noise) 
            latents, sigma = self.scheduler.scale_noise(image_latents, timestep, noise)
            latents = self._pack_latents(latents, batch_size, num_channels_latents, height, width)
        return latents, latent_image_ids, noise, model_input, sigma
    ...
    def get_sigmas(self, timesteps, n_dim=4, dtype=torch.float32):
        ...
        sigmas = noise_scheduler_copy.sigmas.to(device=self._execution_device.device, dtype=dtype)
        schedule_timesteps = noise_scheduler_copy.timesteps.to(self._execution_device.device)
        timesteps = timesteps.to(self._execution_device.device)
    ...
    def __call__(
        ...
        # latents, latent_image_ids, noise, model_input = self.prepare_latents(
        #     init_image,
        #     latent_timestep,
...
# 50
            ...
            self.timesteps = self.scheduler.timesteps[indices].to(device=model_input.device)
        ...
        for sampling_step, t in enumerate(self.timesteps):
            latent_timestep = torch.tensor([t]* batch_size).to(device=device)
            latents, latent_image_ids, noise, model_input, sigmas = self.prepare_latents(
            init_image,
            latent_timestep,
            batch_size * num_images_per_prompt,
            num_channels_latents,
            ...
            # print(latents.shape) # | torch.Size([20, 16, 128, 128])
            # print(timestep.shape) # torch.Size([20])
            # print(guidance.shape) # torch.Size([20])
            # print(text_ids.shape) # torch.Size([512, 3])
            # print(latent_image_ids.shape) # torch.Size([4096, 3])
            # exit(0)
            ...
            # [2025-05-11T13:42:57.792711Z] 4f6b6edd || RuntimeError: mat1 and mat2 shapes cannot be multiplied (40960x128 and 64x3072)
# [2025-05-11T13:42:59.500362Z]          || ERROR: crashed: resources failed with non-zero exit code: container failed with non-zero exit code: 1 (exit code 1)
# [2025-05-11T13:42:59.504068Z]          || ERROR: Command (infinitely-balanced-elf) was terminated: allocation failed: resources failed with non-zero exit code: container failed with non-zero exit code: 1 (exit code 1)
            ...
            if self.guidance_scale <=1:
                ...
                if use_normed_classifier:
                    dist = weighting.float().flatten() * torch.norm((model_pred - target).reshape(target.shape[0], -1), p=2, dim=1)
                else:
                    dist = torch.mean(
                ...
                raise ValueError(
                    f"Guidance scale {guidance_scale} is not supported for this pipeline. Please set guidance_scale <= 1."
                )
