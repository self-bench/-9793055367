...
from ...utils.p2p_utils import AttentionStore, register_attention_control
...
class StableDiffusionImg2ImgPipeline_test(
    DiffusionPipeline,
    ...
    def get_timesteps(self, num_inference_steps, strength, device):
        ...
        # print(init_timestep) # 40
        # input()
        t_start = max(num_inference_steps - init_timestep, 0) 
        print(self.scheduler.timesteps) # 999~0
        timesteps = self.scheduler.timesteps[t_start * self.scheduler.order :]
        # print(timesteps) # 989~0
        if hasattr(self.scheduler, "set_begin_index"):
            self.scheduler.set_begin_index(t_start * self.scheduler.order)

        return timesteps, num_inference_steps - t_start

    def prepare_latents(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None, sampling_step=None, unconditional=False):
...
#           0.0000], device='cuda:0')
        ...
        # with self.progress_bar(total=num_inference_steps) as progress_bar:
            # for i, t in enumerate(timesteps):
                ...
                latent_model_input = torch.cat([latents] * 2) if guidance_scale > 0 or unconditional else latents
                ...
                for j in range(len(prompt)):
                    ...
                    for location in from_where:
                        for item in attention_maps[f"{location}_{'cross' if cross else 'self'}"]:
                            if item.shape[1] == num_pixels:
                                out_cross = []
                                
                                cross_maps = item.reshape(len(prompt), -1, res, res, item.shape[-1])[j]
                                out_cross.append(cross_maps)
                                out.append(cross_maps)
                    out = torch.cat(out, dim=0)
                    out = out.sum(0) / out.shape[0]
                    dist.append(out.unsqueeze(0))

                        # dist = out.sum(0) / out.shape[0]
                        # out = out.cpu().numpy()

                dist = torch.cat(dist, dim=0)
                dist = torch.mean(dist.flatten(1),dim=(1))
                # print(dist.shape)
                # exit(0)
                dists.append(dist)
                ...
                # perform guidance
                # if unconditional or self.do_classifier_free_guidance:
