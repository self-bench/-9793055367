...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        # print(processor) # AttnProcessor2_0
    ...
    def set_processor(self, processor: "AttnProcessor") -> None:
        ...
        # print("processor", processor) # <diffusers.models.attention_processor.JointAttnProcessor2_0
        # exit(0)
...
class AttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # print((query @ key.transpose(-2, -1)).shape) # torch.Size([4, 5, 4096, 77])
        # print(attention_mask)
        # exit()
        # print(query.shape, key.shape, value.shape, hidden_states.shape)
        # torch.Size([4, 5, 4096, 64]) torch.Size([4, 5, 4096, 64]) torch.Size([4, 5, 4096, 64]) torch.Size([4, 5, 4096, 64]) self attn?
        # torch.Size([4, 5, 4096, 64]) torch.Size([4, 5, 77, 64]) torch.Size([4, 5, 77, 64]) torch.Size([4, 5, 4096, 64]) 
        # torch.Size([4, 10, 1024, 64]) torch.Size([4, 10, 77, 64]) torch.Size([4, 10, 77, 64]) torch.Size([4, 10, 1024, 64])                                                        
        # torch.Size([4, 20, 256, 64]) torch.Size([4, 20, 256, 64]) torch.Size([4, 20, 256, 64]) torch.Size([4, 20, 256, 64])  
        ...
        #     rows, cols = 1, num_samples  # Adjust according to the desired grid
        ...
        #         attn_visual = torch.norm(hidden_states[i],dim=0).cpu().numpy()
        #         axes[i].imshow(attn_visual, cmap=white_to_red)  # Show sample in the first row
        #         axes[i].axis('off')
        #         # axes[0, i].set_title(f"{prompt[i]}", fontsize=8)

        #     plt.tight_layout()
        #     task_name = "stl10_crossattn_timestep_10"
        #     os.makedirs(f"{task_name}_images", exist_ok=True)
        #     fig.savefig(f"{task_name}_images/attn_{self.iteration}.png", dpi=300, bbox_inches='tight')  # Save the first prompt's name
